...
class Encoder(nn.Module):
    ...
    def forward(self, blocks, corrupt=False):
        if corrupt:
            for block in blocks:
                block.srcdata['feat'] = block.srcdata['feat'][torch.randperm(block.num_src_nodes())]
        if self.gnn_encoder == 'gcn':
            features = self.conv.forward(blocks)
...
class GGD(nn.Module):
    ...
    def forward(self, features, h_3,loss_func):
    ...
    def encode_for_gTrain(self,features):
        self.eval()
        with torch.no_grad():
            h_1 = self.encoder(features)
        return h_1
